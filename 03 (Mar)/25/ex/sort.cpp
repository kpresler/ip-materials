...
int main( void )
{
  ...
  // PART ? TO DO: Use the implementation of sort from STL to sort the contents of "vec2"
...
void sort( std::vector< int > *values )
{
  // PART ? TO DO: Copy your implementation of the sort function from ex8-1 here
  if( values->size()>1 )
    {
      std::vector< int > left( values->size()/2 );
      std::vector< int > right( values->size() - values->size()/2 );
      
      for( size_t i=0 ; i<values->size()/2 ; i++ )
        left[i] = (*values)[i];
      for( size_t i=values->size()/2 ; i<values->size() ; i++ )
        right[ i-values->size()/2 ] = (*values)[i];
