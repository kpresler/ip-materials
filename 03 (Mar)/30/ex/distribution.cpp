...
// PART 2 TODO: Declare a function that converts a std::vector< double > from a probability density function to a cumulative distribution function.
...
std::vector< double >::const_iterator naive_find_last_iterator( std::vector< double >::const_iterator begin , std::vector< double >::const_iterator end , double v )
{
  ...
  for( ; iter!=end ; iter++ )
    {
      if( *iter<=v && ( iter+1==end || v<=*(iter+1) ) ) return iter;
    }
  return iter;
}
