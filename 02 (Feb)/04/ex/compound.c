...
int main(int argc, char *argv[]) {
  ...
  // open output, make sure it's fine too
  ...
  // check that we read in the two values expected
  while ( ( parse = fscanf(inFile, "%f%f", &p, &r) ) == 2) {
  ...
  // we weren't able to read both elements we expected out of a line in the file
  if (parse != 2 && parse != EOF) {
    printf("Error: error in parsing the input file\n");
    return 4;
  }

  // something bad happened reading input file
  if (ferror(inFile)) {
    printf("Error: error indicator was set for input file\n");
    return 5;
  }
  // something bad happened writing output file
  if (ferror(outFile)) {
