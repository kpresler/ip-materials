...
class Weapon {
    ...
    public:
        ...
        virtual int damageFalloff(int distance) { return 0; }
...
class MeleeWeapon : public Weapon {
    ...
    public:
        MeleeWeapon (int damage, int length): Weapon (damage, false), length (length) {}

        virtual int damageFalloff(int distance) override {
...
class Sword : public MeleeWeapon {
    ...
    public:
        Sword (int damage, int numHands, int length): MeleeWeapon (damage, length), numHands (numHands) {}
...
class LaserCannon : public Weapon {
    public:
        LaserCannon (int damage): Weapon(damage, true) {}

        virtual int damageFalloff (int distance) override { return damage; } // lasers do not obey laws of physics and suffer no falloff
...
int main (void) {
    ...
    attackEnemy(broadSword, 3);

    attackEnemy(plc, 100);

    Weapon unspecified (30, false);
